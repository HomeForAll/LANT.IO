...
class SphinxClient
{
    ...
    function _Connect()
    {
        ...
        if ($this->_timeout <= 0)
            $fp = @fsockopen($host, $port, $errno, $errstr); else
            $fp = @fsockopen($host, $port, $errno, $errstr, $this->_timeout);
        
        if (!$fp) {
            if ($this->_path)
                $location = $this->_path; else
    ...
    function _ParseSearchResponse($response, $nreqs)
    {
        ...
        for ($ires = 0; $ires < $nreqs && $p < $max; $ires++) {
            ...
            while ($count-- > 0 && $p < $max) {
                ...
                if ($this->_arrayresult)
                    $result["matches"][$idx]["attrs"] = $attrvals; else
    ...
    function FlushAttributes()
    {
        ...
        if (strlen($response) == 4)
            list(, $tag) = unpack("N*", $response); else
