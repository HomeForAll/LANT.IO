...
class CabinetModel extends Model
{
    ...
    public function numberofpages()
    {
        ...
        if (count($array) % self::IDSPERPAGE == 0)
            $result = count($array) / self::IDSPERPAGE; else
        ...
        if (isset($_POST['showdb'])) {
            ...
            if (isset($_POST['showdb'])) {
                ...
                foreach ($array as $info) {
                    $part = "ID - {$info['id']}" . '<br>' . "Key - {$info['key']}" . '<br>' . "Email - {$info['email']}" . '<br>' . "Email_sent - {$info['email_sent']}" . '<br>' . "Creation_date - {$info['creation_date']}" . '<br>' . "Inactive date - {$info['inactive_date']}" . '<br>';
    ...
    public function keyeditor()
    {
        if (isset($_POST['idworkgo'])) {
            ...
            if (isset($array[$id_key])) {
                $part = "ID - {$array[$id_key]['id']}" . '<br>' . "Key - {$array[$id_key]['key']}" . '<br>' . "Email - {$array[$id_key]['email']}" . '<br>' . "Email_sent - {$array[$id_key]['email_sent']}" . '<br>' . "Creation_date - {$array[$id_key]['creation_date']}" . '<br>' . "Inactive date - {$array[$id_key]['inactive_date']}" . '<br>';
